...
impl mydsp {
	...
	fn instance_clear(&mut self) {
		...
		for l18 in 0..2 {
			self.fRec10[l18 as usize] = 0.0;
		}
		for l19 in 0..2 {
			self.iRec13[l19 as usize] = 0;
		}
		for l20 in 0..2 {
		...
		for l22 in 0..2 {
			self.fRec14[l22 as usize] = 0.0;
		}
		for l23 in 0..2 {
			self.fVec10[l23 as usize] = 0.0;
		}
		for l24 in 0..2 {
			self.fRec15[l24 as usize] = 0.0;
		}
		for l25 in 0..2 {
		...
		for l27 in 0..2 {
			self.fRec16[l27 as usize] = 0.0;
		}
		for l28 in 0..2 {
			self.iRec18[l28 as usize] = 0;
		}
		for l29 in 0..2 {
	...
	fn instance_constants(&mut self, sample_rate: i32) {
		...
		self.fConst3 = (0.25 * self.fConst0);
		self.fConst4 = (1.0 / self.fConst0);
		self.fConst5 = (0.5 * self.fConst0);
		self.fConst6 = (3.0 / self.fConst0);
		self.fConst7 = (2.0 * self.fConst0);
	...
	unsafe fn compute(&mut self, count: i32, inputs: &[T], outputs: &mut [&mut [T];1]) {
		...
		let mut fSlow12: f32 = (0.00100000005 * (self.fHslider7 as f32));
		let mut fSlow13: f32 = (1.0 / fSlow10);
		let mut fSlow14: f32 = (self.fConst0 / fSlow10);
		let mut fSlow15: f32 = (self.fConst4 * fSlow10);
		let mut fSlow16: f32 = (0.00100000005 * (self.fHslider8 as f32));
		let mut fSlow17: f32 = f32::max(fSlow10, 23.4489498);
		let mut fSlow18: f32 = f32::max(20.0, f32::abs(fSlow17));
		let mut fSlow19: f32 = (self.fConst3 / fSlow18);
		let mut fSlow20: f32 = (self.fConst0 / fSlow17);
		let mut fSlow21: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst5 / fSlow17)));
		let mut fSlow22: f32 = f32::floor(fSlow21);
		let mut fSlow23: f32 = (fSlow22 + (1.0 - fSlow21));
		let mut iSlow24: i32 = (fSlow21 as i32);
		let mut fSlow25: f32 = (fSlow21 - fSlow22);
		let mut iSlow26: i32 = (iSlow24 + 1);
		let mut iSlow27: i32 = (fSlow14 as i32);
		let mut fSlow28: f32 = f32::max((0.5 * fSlow10), 23.4489498);
		let mut fSlow29: f32 = f32::max(20.0, f32::abs(fSlow28));
		let mut fSlow30: f32 = (self.fConst3 / fSlow29);
		let mut fSlow31: f32 = (self.fConst0 / fSlow28);
		let mut fSlow32: f32 = (self.fConst6 * fSlow10);
		let mut fSlow33: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst5 / fSlow28)));
		...
		let mut iSlow39: i32 = ((self.fConst7 / fSlow10) as i32);
		...
		let mut fSlow47: f32 = (self.fConst3 / fSlow46);
		let mut fSlow48: f32 = (self.fConst0 / fSlow45);
		let mut fSlow49: f32 = (self.fConst6 * fSlow44);
		...
		let mut iSlow57: i32 = ((self.fConst7 / fSlow44) as i32);
		let mut fSlow58: f32 = (self.fConst4 * fSlow44);
		let mut fSlow59: f32 = (1.0 / fSlow44);
		let mut fSlow60: f32 = (self.fConst2 * fSlow44);
		let mut fSlow61: f32 = f32::max(fSlow44, 23.4489498);
		let mut fSlow62: f32 = f32::max(20.0, f32::abs(fSlow61));
		let mut fSlow63: f32 = (self.fConst3 / fSlow62);
		...
		for output0 in zipped_iterators {
			...
			let mut fTemp2: f32 = (((self.fRec4[0] < 2.0) as i32) as f32);
			self.fVec1[0] = 0.25;
			let mut fTemp3: f32 = (self.iVec0[1] as f32);
			let mut iTemp4: i32 = ((self.fRec4[0] >= 3.0) as i32);
			let mut fTemp5: f32 = if ((((self.fRec4[0] == 0.0) as i32) + iTemp4) as i32 != 0) { 1.0 } else { f32::max(f32::max(1.0, ((0.0199999996 * (self.fRec4[0] + -2.0999999)) + 1.0)), ((0.0199999996 * (1.0 - self.fRec4[0])) + 1.0)) };
			let mut fTemp6: f32 = f32::max((fSlow10 * fTemp5), 23.4489498);
			let mut fTemp7: f32 = f32::max(20.0, f32::abs(fTemp6));
			self.fVec2[0] = fTemp7;
			let mut fTemp8: f32 = (self.fRec6[1] + (self.fConst4 * self.fVec2[1]));
			self.fRec6[0] = (fTemp8 - f32::floor(fTemp8));
			let mut fTemp9: f32 = mydsp_faustpower2_f(((2.0 * self.fRec6[0]) + -1.0));
			self.fVec3[0] = fTemp9;
			let mut fTemp10: f32 = ((fTemp3 * (fTemp9 - self.fVec3[1])) / fTemp7);
			self.fVec4[(self.IOTA & 4095) as usize] = fTemp10;
			let mut fTemp11: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst5 / fTemp6)));
			let mut iTemp12: i32 = (fTemp11 as i32);
			let mut fTemp13: f32 = f32::floor(fTemp11);
			self.fRec5[0] = ((0.999000013 * self.fRec5[1]) + (self.fConst3 * ((fTemp10 - (self.fVec4[((self.IOTA - iTemp12) & 4095) as usize] * (fTemp13 + (1.0 - fTemp11)))) - ((fTemp11 - fTemp13) * self.fVec4[((self.IOTA - (iTemp12 + 1)) & 4095) as usize]))));
			let mut fTemp14: f32 = (self.fRec5[0] * fTemp5);
			self.fRec7[0] = (fSlow12 + (0.999000013 * self.fRec7[1]));
			let mut fTemp15: f32 = f32::min(1.0, f32::max(0.0, (2.0 - self.fRec7[0])));
			let mut fTemp16: f32 = f32::min(1.0, f32::max(0.0, (self.fRec7[0] + -2.0)));
			let mut fTemp17: f32 = (1.0 - (fTemp15 + fTemp16));
			let mut fTemp18: f32 = (self.fVec1[1] * fTemp16);
			self.iRec9[0] = ((self.iVec0[1] + self.iRec9[1]) % ((fSlow14 / fTemp5) as i32));
			let mut fTemp19: f32 = (0.100000001 * (f32::max(3.0, self.fRec7[0]) + -3.0));
			let mut fTemp20: f32 = (fTemp5 + fTemp19);
			let mut fTemp21: f32 = ((self.fRec8[1] * (1.0 - (((((self.iRec9[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow15 * fTemp20));
			self.fRec8[0] = (fTemp21 - f32::floor(fTemp21));
			let mut fTemp22: f32 = mydsp_faustpower2_f(((2.0 * self.fRec8[0]) + -1.0));
			self.fVec5[0] = fTemp22;
			let mut fTemp23: f32 = f32::min(0.5, (0.5 * self.fRec7[0]));
			let mut fTemp24: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst0 * (fTemp23 / fTemp6))));
			let mut iTemp25: i32 = (fTemp24 as i32);
			let mut fTemp26: f32 = f32::floor(fTemp24);
			let mut fTemp27: f32 = ((fSlow13 * ((fTemp18 * (fTemp22 - self.fVec5[1])) / fTemp20)) + (0.25 * (fTemp15 * ((fTemp10 - (self.fVec4[((self.IOTA - iTemp25) & 4095) as usize] * (fTemp26 + (1.0 - fTemp24)))) - ((fTemp24 - fTemp26) * self.fVec4[((self.IOTA - (iTemp25 + 1)) & 4095) as usize])))));
			let mut fTemp28: f32 = f32::min(1.0, f32::max(0.0, (2.0 - self.fRec4[0])));
			let mut fTemp29: f32 = (1.0 - fTemp28);
			let mut fTemp30: f32 = if (iTemp4 as i32 != 0) { 1.49829996 } else { 1.0 };
			let mut fTemp31: f32 = (fTemp30 / fTemp5);
			let mut fTemp32: f32 = f32::max((fSlow10 * fTemp31), 23.4489498);
			let mut fTemp33: f32 = f32::max(20.0, f32::abs(fTemp32));
			self.fVec6[0] = fTemp33;
			let mut fTemp34: f32 = (self.fRec11[1] + (self.fConst4 * self.fVec6[1]));
			self.fRec11[0] = (fTemp34 - f32::floor(fTemp34));
			let mut fTemp35: f32 = mydsp_faustpower2_f(((2.0 * self.fRec11[0]) + -1.0));
			self.fVec7[0] = fTemp35;
			let mut fTemp36: f32 = ((fTemp3 * (fTemp35 - self.fVec7[1])) / fTemp33);
			self.fVec8[(self.IOTA & 4095) as usize] = fTemp36;
			let mut fTemp37: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst5 / fTemp32)));
			let mut iTemp38: i32 = (fTemp37 as i32);
			let mut fTemp39: f32 = f32::floor(fTemp37);
			self.fRec10[0] = ((0.999000013 * self.fRec10[1]) + (self.fConst3 * ((fTemp36 - (self.fVec8[((self.IOTA - iTemp38) & 4095) as usize] * (fTemp39 + (1.0 - fTemp37)))) - ((fTemp37 - fTemp39) * self.fVec8[((self.IOTA - (iTemp38 + 1)) & 4095) as usize]))));
			let mut fTemp40: f32 = (self.fRec10[0] * fTemp30);
			let mut fTemp41: f32 = (fTemp5 / fTemp30);
			self.iRec13[0] = ((self.iVec0[1] + self.iRec13[1]) % ((fSlow14 * fTemp41) as i32));
			let mut fTemp42: f32 = (fTemp19 + fTemp31);
			let mut fTemp43: f32 = ((self.fRec12[1] * (1.0 - (((((self.iRec13[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow15 * fTemp42));
			self.fRec12[0] = (fTemp43 - f32::floor(fTemp43));
			let mut fTemp44: f32 = mydsp_faustpower2_f(((2.0 * self.fRec12[0]) + -1.0));
			self.fVec9[0] = fTemp44;
			let mut fTemp45: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst0 * (fTemp23 / fTemp32))));
			let mut iTemp46: i32 = (fTemp45 as i32);
			let mut fTemp47: f32 = f32::floor(fTemp45);
			let mut fTemp48: f32 = ((fSlow13 * ((fTemp18 * (fTemp44 - self.fVec9[1])) / fTemp42)) + (0.25 * (fTemp15 * ((fTemp36 - (self.fVec8[((self.IOTA - iTemp46) & 4095) as usize] * (fTemp47 + (1.0 - fTemp45)))) - ((fTemp45 - fTemp47) * self.fVec8[((self.IOTA - (iTemp46 + 1)) & 4095) as usize])))));
			let mut fTemp49: f32 = (((self.fRec4[0] >= 2.0) as i32) as f32);
			self.fRec14[0] = (fSlow16 + (0.999000013 * self.fRec14[1]));
			let mut fTemp50: f32 = (1.0 - self.fRec14[0]);
			self.fVec10[0] = fSlow18;
			let mut fTemp51: f32 = (self.fRec15[1] + (self.fConst4 * self.fVec10[1]));
			self.fRec15[0] = (fTemp51 - f32::floor(fTemp51));
			let mut fTemp52: f32 = mydsp_faustpower2_f(((2.0 * self.fRec15[0]) + -1.0));
			self.fVec11[0] = fTemp52;
			let mut fTemp53: f32 = (fSlow19 * (fTemp3 * (fTemp52 - self.fVec11[1])));
			self.fVec12[(self.IOTA & 4095) as usize] = fTemp53;
			let mut fTemp54: f32 = f32::max(0.0, f32::min(2047.0, (fSlow20 * fTemp23)));
			let mut iTemp55: i32 = (fTemp54 as i32);
			let mut fTemp56: f32 = f32::floor(fTemp54);
			self.fRec16[0] = ((fTemp53 + (0.999000013 * self.fRec16[1])) - ((fSlow23 * self.fVec12[((self.IOTA - iSlow24) & 4095) as usize]) + (fSlow25 * self.fVec12[((self.IOTA - iSlow26) & 4095) as usize])));
			self.iRec18[0] = ((self.iVec0[1] + self.iRec18[1]) % iSlow27);
			let mut fTemp57: f32 = (fTemp19 + 1.0);
			let mut fTemp58: f32 = ((self.fRec17[1] * (1.0 - (((((self.iRec18[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow15 * fTemp57));
			self.fRec17[0] = (fTemp58 - f32::floor(fTemp58));
			let mut fTemp59: f32 = mydsp_faustpower2_f(((2.0 * self.fRec17[0]) + -1.0));
			self.fVec13[0] = fTemp59;
			self.fVec14[0] = fSlow29;
			let mut fTemp60: f32 = (self.fRec19[1] + (self.fConst4 * self.fVec14[1]));
			...
			let mut fTemp62: f32 = (fSlow30 * (fTemp3 * (fTemp61 - self.fVec15[1])));
			self.fVec16[(self.IOTA & 4095) as usize] = fTemp62;
			let mut fTemp63: f32 = f32::max(0.0, f32::min(2047.0, (fSlow31 * fTemp23)));
			...
			let mut fTemp66: f32 = (fTemp19 + 0.5);
			let mut fTemp67: f32 = ((self.fRec21[1] * (1.0 - (((((self.iRec22[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow15 * fTemp66));
			...
			self.fRec3[0] = ((((fTemp2 * ((fSlow11 * (fTemp14 * fTemp17)) + (self.fConst0 * fTemp27))) * (fTemp29 + (fTemp28 * ((fSlow11 * ((fTemp40 * fTemp17) / fTemp5)) + (self.fConst0 * fTemp48))))) + ((fTemp49 * ((self.fConst0 * (fTemp27 + fTemp48)) + (fSlow11 * (fTemp17 * (fTemp14 + (fTemp40 / fTemp5)))))) + ((fTemp50 * ((fTemp15 * (fTemp53 - ((self.fVec12[((self.IOTA - iTemp55) & 4095) as usize] * (fTemp56 + (1.0 - fTemp54))) + ((fTemp54 - fTemp56) * self.fVec12[((self.IOTA - (iTemp55 + 1)) & 4095) as usize])))) + ((fSlow11 * (self.fRec16[0] * fTemp17)) + (fSlow14 * ((fTemp18 * (fTemp59 - self.fVec13[1])) / fTemp57))))) + (self.fRec14[0] * (((fTemp15 * (fTemp62 - ((self.fVec16[((self.IOTA - iTemp64) & 4095) as usize] * (fTemp65 + (1.0 - fTemp63))) + ((fTemp63 - fTemp65) * self.fVec16[((self.IOTA - (iTemp64 + 1)) & 4095) as usize])))) + (fSlow32 * (self.fRec20[0] * fTemp17))) + (fSlow14 * ((fTemp18 * (fTemp68 - self.fVec17[1])) / fTemp66))))))) - (((self.fRec3[2] * fTemp71) + (2.0 * (self.fRec3[1] * fTemp72))) / fTemp73));
			...
			let mut fTemp76: f32 = (self.fRec28[1] + (self.fConst4 * self.fVec18[1]));
			...
			let mut fTemp79: f32 = f32::max(0.0, f32::min(2047.0, (fSlow48 * fTemp23)));
			...
			let mut fTemp82: f32 = ((self.fRec30[1] * (1.0 - (((((self.iRec31[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow58 * fTemp66));
			...
			self.iRec33[0] = ((self.iVec0[1] + self.iRec33[1]) % ((fSlow56 / fTemp5) as i32));
			let mut fTemp84: f32 = ((self.fRec32[1] * (1.0 - (((((self.iRec33[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow58 * fTemp20));
			...
			let mut fTemp86: f32 = f32::max((fSlow44 * fTemp5), 23.4489498);
			let mut fTemp87: f32 = f32::max(20.0, f32::abs(fTemp86));
			self.fVec23[0] = fTemp87;
			let mut fTemp88: f32 = (self.fRec34[1] + (self.fConst4 * self.fVec23[1]));
			...
			let mut fTemp91: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst0 * (fTemp23 / fTemp86))));
			let mut iTemp92: i32 = (fTemp91 as i32);
			let mut fTemp93: f32 = f32::floor(fTemp91);
			let mut fTemp94: f32 = ((fSlow59 * ((fTemp18 * (fTemp85 - self.fVec22[1])) / fTemp20)) + (0.25 * (fTemp15 * ((fTemp90 - (self.fVec25[((self.IOTA - iTemp92) & 4095) as usize] * (fTemp93 + (1.0 - fTemp91)))) - ((fTemp91 - fTemp93) * self.fVec25[((self.IOTA - (iTemp92 + 1)) & 4095) as usize])))));
			self.iRec36[0] = ((self.iVec0[1] + self.iRec36[1]) % ((fSlow56 * fTemp41) as i32));
			let mut fTemp95: f32 = ((self.fRec35[1] * (1.0 - (((((self.iRec36[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow58 * fTemp42));
			...
			let mut fTemp97: f32 = f32::max((fSlow44 * fTemp31), 23.4489498);
			let mut fTemp98: f32 = f32::max(20.0, f32::abs(fTemp97));
			self.fVec27[0] = fTemp98;
			let mut fTemp99: f32 = (self.fRec37[1] + (self.fConst4 * self.fVec27[1]));
			...
			let mut fTemp102: f32 = f32::max(0.0, f32::min(2047.0, (self.fConst0 * (fTemp23 / fTemp97))));
			let mut iTemp103: i32 = (fTemp102 as i32);
			let mut fTemp104: f32 = f32::floor(fTemp102);
			let mut fTemp105: f32 = ((fSlow59 * ((fTemp18 * (fTemp96 - self.fVec26[1])) / fTemp42)) + (0.25 * (fTemp15 * ((fTemp101 - (self.fVec29[((self.IOTA - iTemp103) & 4095) as usize] * (fTemp104 + (1.0 - fTemp102)))) - ((fTemp102 - fTemp104) * self.fVec29[((self.IOTA - (iTemp103 + 1)) & 4095) as usize])))));
			...
			self.fRec38[0] = ((0.999000013 * self.fRec38[1]) + (self.fConst3 * ((fTemp90 - (self.fVec25[((self.IOTA - iTemp107) & 4095) as usize] * (fTemp108 + (1.0 - fTemp106)))) - ((fTemp106 - fTemp108) * self.fVec25[((self.IOTA - (iTemp107 + 1)) & 4095) as usize]))));
			let mut fTemp109: f32 = (self.fRec38[0] * fTemp5);
			...
			self.fRec39[0] = ((0.999000013 * self.fRec39[1]) + (self.fConst3 * ((fTemp101 - (self.fVec29[((self.IOTA - iTemp111) & 4095) as usize] * (fTemp112 + (1.0 - fTemp110)))) - ((fTemp110 - fTemp112) * self.fVec29[((self.IOTA - (iTemp111 + 1)) & 4095) as usize]))));
			let mut fTemp113: f32 = (self.fRec39[0] * fTemp30);
			self.fVec30[0] = fSlow62;
			let mut fTemp114: f32 = (self.fRec40[1] + (self.fConst4 * self.fVec30[1]));
			...
			let mut fTemp117: f32 = f32::max(0.0, f32::min(2047.0, (fSlow64 * fTemp23)));
			...
			let mut fTemp120: f32 = ((self.fRec42[1] * (1.0 - (((((self.iRec43[0] == 0) as i32) > 0) as i32) as f32))) + (fSlow58 * fTemp57));
			...
			self.fRec27[0] = (((self.fRec14[0] * (((fTemp15 * (fTemp78 - ((self.fVec20[((self.IOTA - iTemp80) & 4095) as usize] * (fTemp81 + (1.0 - fTemp79))) + ((fTemp79 - fTemp81) * self.fVec20[((self.IOTA - (iTemp80 + 1)) & 4095) as usize])))) + (fSlow49 * (self.fRec29[0] * fTemp17))) + (fSlow56 * ((fTemp18 * (fTemp83 - self.fVec21[1])) / fTemp66)))) + (((fTemp49 * ((self.fConst0 * (fTemp94 + fTemp105)) + (fSlow60 * (fTemp17 * (fTemp109 + (fTemp113 / fTemp5)))))) + ((fTemp2 * ((fSlow60 * (fTemp109 * fTemp17)) + (self.fConst0 * fTemp94))) * (fTemp29 + (fTemp28 * ((fSlow60 * ((fTemp113 * fTemp17) / fTemp5)) + (self.fConst0 * fTemp105)))))) + (fTemp50 * (((fTemp15 * (fTemp116 - ((self.fVec32[((self.IOTA - iTemp118) & 4095) as usize] * (fTemp119 + (1.0 - fTemp117))) + ((fTemp117 - fTemp119) * self.fVec32[((self.IOTA - (iTemp118 + 1)) & 4095) as usize])))) + (fSlow60 * (self.fRec41[0] * fTemp17))) + (fSlow56 * ((fTemp18 * (fTemp121 - self.fVec33[1])) / fTemp57)))))) - (((fTemp71 * self.fRec27[2]) + (2.0 * (fTemp72 * self.fRec27[1]))) / fTemp73));
